(function() {
  ...
  function init() {
	...
	ninth: "images/ninth.png"
      ...
      var srcLength = (Object.keys(imgSources).length)+1;
      for(num=1;num<srcLength;num++) {
        window["img"+num] = new Image();
      }
    ...
    img1.addEventListener("load", function() {
      ...
      physics.collision();
      // Create some walls
      new Body(physics, { color: "red", type: "static", x: .2, y: 0, height: 50,  width: 0.5 });
      new Body(physics, { color: "red", type: "static", x:51, y: 0, height: 50,  width: 0.5});
      // Sides
      new Body(physics, { color: "red", type: "static", x: 0, y: .2, height: 0.5, width: 102 });
      new Body(physics, { color: "red", type: "static", x: 0, y:25, height: 0.5, width: 102.5 });
 
      var lengthSrc = 1;
      for( var key in imgSources ) {
        
            var axisY = Math.round(Math.random() * 14);
            var axisX = Math.floor(Math.random() * 45);
            new Body(physics, { image: window["img"+lengthSrc], x: axisX, y: axisY });
            lengthSrc++;
...
*/
 ...
 var lengthSource = 1;
 for( var keySource in imgSources ) {
        if( imgSources.hasOwnProperty(keySource) ) {
            window["img"+lengthSource].src = imgSources[keySource];
            lengthSource++;
        }
    }
  }
